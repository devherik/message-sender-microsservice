...
class TestMessageAPIEndToEnd:
    ...
    async def test_create_message_full_workflow(self, api_client):
        ...
        # Arrange
        app_id = 1
        payload = {"message_content": "E2E test message", "phone_number_id": 1}

        # Act
        response = await api_client.post(f"/api/messages/{app_id}", json=payload)
    ...
    async def test_create_message_validates_request_body(self, api_client):
        ...
        response = await api_client.post(
            f"/api/messages/{app_id}", json=invalid_payload
        )
